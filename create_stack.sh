...
for i in $(seq $START_CNT $END_CNT); do
    aws cloudformation create-stack --stack-name ec2-test-${i} --template-body file://kpi-ec2.yml --parameters ParameterKey=AMI_ID,ParameterValue=${AMI} ParameterKey=InstanceName,ParameterValue=ec2-test-${i}
done